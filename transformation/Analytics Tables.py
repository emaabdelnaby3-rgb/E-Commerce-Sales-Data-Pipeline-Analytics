...
order_items = pd.read_csv(PROCESSED_PATH / "order_items_cleaned.csv", parse_dates=["shipping_limit_date"])
...
reviews = pd.read_csv(PROCESSED_PATH / "reviews_cleaned.csv", parse_dates=["review_creation_date", "review_answer_timestamp"])
...
order_items_summary["order_items_total_value"] = order_items_summary["total_product_value"] + order_items_summary["total_freight_value"]
...
fact_orders = fact_orders.merge(payment_summary, on="order_id", how="left")
fact_orders = fact_orders.merge(order_items_summary, on="order_id", how="left")
fact_orders = fact_orders.merge(review_summary, on="order_id", how="left")
...
fact_orders["order_month_date"] = fact_orders["order_purchase_timestamp"].dt.to_period("M").dt.to_timestamp()
fact_orders["is_delivered"] = fact_orders["order_status"].apply(lambda x: 1 if x == "delivered" else 0)
fact_orders["is_cancelled"] = fact_orders["order_status"].apply(lambda x: 1 if x == "canceled" else 0)
...
fact_order_items = order_items.merge(
    orders[[
        ...
        "delivery_days", "delivery_delay_days", "is_delayed"
    ]],
    on="order_id",
    how="left"
)
...
fact_order_items = fact_order_items.merge(review_summary, on="order_id", how="left")
...
monthly_sales["delay_rate"] = (monthly_sales["delayed_orders"] / monthly_sales["total_orders"]).round(4)
...
# ==============================
customer_analytics = delivered_orders.groupby(["customer_unique_id", "customer_city", "customer_state"]).agg(
    total_orders=("order_id", "nunique"),
...
customer_analytics["delay_rate"] = (customer_analytics["delayed_orders"] / customer_analytics["total_orders"]).round(4)
...
# ==============================
delivered_items = fact_order_items[fact_order_items["order_status"] == "delivered"].copy()

product_analytics = delivered_items.groupby(["product_id", "product_category_name_english"]).agg(
...
# ==============================
category_analytics = delivered_items.groupby("product_category_name_english").agg(
    total_orders=("order_id", "nunique"),
...
# ==============================
delivery_performance = delivered_orders.groupby(["customer_state"]).agg(
    total_orders=("order_id", "nunique"),
    avg_delivery_days=("delivery_days", "mean"),
...
delivery_performance["delay_rate"] = (delivery_performance["delayed_orders"] / delivery_performance["total_orders"]).round(4)
delivery_performance = delivery_performance.sort_values(by="delay_rate", ascending=False)
...
# ==============================
payments_with_orders = payments.merge(orders[["order_id", "order_status"]], on="order_id", how="left")
payments_with_orders = payments_with_orders[payments_with_orders["order_status"] == "delivered"].copy()
